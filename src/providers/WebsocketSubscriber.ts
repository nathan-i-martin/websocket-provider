...
export class WebsocketSubscriber implements WebSocketHandler {
    ...
    on = (eventName: string, callback: Function): void => {
        ...
        if(eventName == "reconnect")    return this.#_onReconnect(callback as OnReconnectCallback);
    ...
    connect = (): Promise<boolean> => new Promise((success) => {
        ...
        this.#_connection.on("open",(connection: WebSocket) => {
         ...
         */
         this.#_connection.on("message", (message: Buffer) => {
            this.#_onMessageCallback(message.toString());
        });
